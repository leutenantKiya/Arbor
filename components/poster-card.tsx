...
export function PosterCard({ film }: { film: Film }) {
  return (
    <Link
      href={`/film/${film.slug}`}
      className="group relative w-44 shrink-0 overflow-hidden rounded-card border border-line-soft bg-surface transition-colors hover:border-cream/30 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-amber sm:w-[220px]"
    >
      <Image
        src={film.posterUrl}
      ...
      <span className="absolute right-3 top-3 grid h-8 w-8 place-items-center rounded-full border border-cream/30 bg-bark/55 text-[0.6rem] text-cream opacity-0 backdrop-blur-sm transition-all duration-200 group-hover:scale-100 group-hover:opacity-100">
        ▶
      </span>
