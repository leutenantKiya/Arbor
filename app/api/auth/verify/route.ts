...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error("[verify] Failed to parse request body:", err);
    return NextResponse.json({ error: "Invalid request body" }, { status: 400 });
  }
  if (!parsed.success) {
    console.error("[verify] Zod validation failed:", parsed.error.format());
  ...
  try {
    ...
    console.error("[verify] Particle verification failed:", err);
  ...
  try {
    ...
    console.error("[verify] DB/session error:", err);
